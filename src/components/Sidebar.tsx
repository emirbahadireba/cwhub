  ...
  return (
    <>
      ...
      <motion.div
        ...
        className={`fixed lg:relative lg:translate-x-0 lg:opacity-100 top-0 left-0 w-64 sm:w-72 min-h-screen bg-surface border-r border-border flex flex-col z-50 lg:z-auto ${
          isOpen ? 'shadow-2xl' : ''
        }`}
      >
        ...
        <div className="p-4 sm:p-6">
          <div className="flex items-center space-x-3">
            <div className="w-8 sm:w-10 h-8 sm:h-10 bg-gradient-to-r from-primary to-secondary rounded-xl flex items-center justify-center">
              <PieChart className="w-4 sm:w-6 h-4 sm:h-6 text-white" />
            </div>
            <div>
              <h1 className="text-lg sm:text-xl font-bold text-white">CreativeHub</h1>
            ...
            <AnimatePresence>
              {showUserMenu && (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: -10 }}
                  className="absolute top-full left-4 sm:left-6 right-4 sm:right-6 mt-2 bg-surface border border-border rounded-xl p-4 shadow-lg z-50"
                >
            ...
            <AnimatePresence>
              {showNotifications && (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: -10 }}
                  className="absolute top-full right-4 sm:right-6 mt-2 w-72 sm:w-80 bg-surface border border-border rounded-xl shadow-lg z-50 max-h-96 overflow-y-auto"
                >
            ...
            {menuItems.map((item) => {
              ...
              return (
                <motion.button
                  ...
                  whileHover={{ x: 4 }}
                  whileTap={{ scale: 0.98 }}
                  className={`w-full flex items-center space-x-3 px-3 sm:px-4 py-2 sm:py-3 rounded-xl text-left transition-all ${
                    isActive
                ...
                >
                  <Icon className={`w-4 sm:w-5 h-4 sm:h-5 ${isActive ? 'text-primary' : ''}`} />
        ...
        <div className="p-4 sm:p-6 border-t border-border">
          <motion.button
            whileHover={{ x: 4 }}
            onClick={handleLogout}
            className="w-full flex items-center space-x-3 px-3 sm:px-4 py-2 sm:py-3 rounded-xl text-textSecondary hover:text-white hover:bg-background/30 transition-all"
          >
